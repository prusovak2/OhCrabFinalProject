...
impl OhCrabVisualizer {
    pub fn new(robot_creator: impl FnOnce(VisualizableInterfaces) -> Box<dyn Runnable>  /*robot: Box<dyn Runnable>*/, mut world_generator: OxAgWorldGenerator) -> OhCrabVisualizer {
        let ifaces = VisualizableInterfaces{};
        let robot = robot_creator(ifaces);
